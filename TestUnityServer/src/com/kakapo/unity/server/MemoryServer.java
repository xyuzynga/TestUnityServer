...
import com.kakapo.unity.message.server.MaxConnectionsMessage;
  ...
  implements Server
{
  ...
  public void processScheduledStatuses()
  {
    ...
    while (!this.scheduledStatusUpdates.isEmpty())
    {
      ...
      if (head.scheduledStatus.isValid())
      {
        ...
        Extension extension = group.getExtension(head.extension);
  ...
  private void addScheduledStatusMessageItem(Map<Group, StatusListMessage> messages, Group group, Extension extension, Status status)
  {
  ...
  public void register(Client client, boolean override)
  {
    try
    {
      logger.info("Client: " + client + " Override: " + override);

      if (this.connections >= this.maxConnections)
      {
        logger.info("Maximum connections reached");
        client.receive(new MaxConnectionsMessage(this.redirect));
        return;
      }
      ...
      if (group == null)
      {
        logger.info("Creating new group: " + client.getGroup());
        group = new Group(client.getGroup());
        Group existing = (Group)this.groups.put(client.getGroup(), group);
        assert (existing == null);
      }

      Extension extension = group.getExtension(client.getExtension());
      if (extension != null)
      {
        if (extension.getClient() != null)
        {
          if (override)
          {
            logger.info("Replacing client " + client);
          ...
          {
            logger.info("Extension already connected for " + client);
  ...
  private void sendNewContact(Extension extension, Group group)
  {
    Collection<Extension> extensions = group.getExtensions();
    ...
    Collection<Extension> excluded = new ExclusionSet<Extension>(extensions, extension);

    send(excluded, message);
  }

  private void sendContactList(Extension extension, Group group)
  {
    Collection<Extension> extensions = group.getExtensions();

    Collection<ContactAction> actions = new ArrayList<ContactAction>(extensions.size());
    for (Extension recipient : extensions)
    {
      Client other = recipient.getClient();
  ...
  public void send(Client sender, ClientMessage message)
  {
    if (logger.isLoggable(Level.FINE))
    {
    ...
    try
    {
      if ((message instanceof PeerMessage))
      {
        ...
        Collection<Extension> recipients = new ArrayList<Extension>(extensions.size());
        for (String destination : extensions)
        {
          Extension extension = group.getExtension(destination);
          if (extension != null)
          {
      ...
      {
        Group group = (Group)this.groups.get(sender.getGroup());
        Extension extension = group.getExtension(sender.getExtension());

        if ((message instanceof SetStatusMessage))
        {
          SetStatusMessage usm = (SetStatusMessage)message;
          Extension subject = group.getExtension(usm.getExtension());
        ...
        else if ((message instanceof ClearStatusMessage))
        {
          ...
          Extension subject = group.getExtension(csm.getExtension());
          Status current = subject.getCurrentStatus();
        ...
        else if ((message instanceof AddScheduledStatusMessage))
        {
          ...
          Extension subject = group.getExtension(assm.getExtension());
        ...
        else if ((message instanceof RemoveScheduledStatusMessage))
        {
          ...
          Extension subject = group.getExtension(rssm.getExtension());
        ...
        else if ((message instanceof ListScheduledStatusesMessage))
        {
          ...
          Extension subject = group.getExtension(lssm.getExtension());
  ...
  protected void send(Collection<Extension> extensions, Message message)
  {
  ...
  private void send(Extension recipient, Message message)
  {
    send(new SingleItemSet<Extension>(recipient), message);
  }
  ...
  public void shutdown()
  {
    ...
    Collection<Extension> clients = new ArrayList<Extension>(this.connections);
